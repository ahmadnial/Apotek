...
<div class="pull-right">
    {{ link_to_route('transactions.pdf', trans('transaction.invoice_pdf'), [$transaction->invoice_no], ['class' => 'btn
    btn-default']) }}
    {{ link_to_route('transactions.receipt', trans('transaction.invoice_print'), [$transaction->invoice_no], ['class' =>
    'btn btn-default']) }}
</div>
    ...
    <div class="col-sm-4">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">{{ trans('transaction.detail') }}</h3>
            </div>
                ...
                <table class="table table-condensed">
                    <tbody>
                        <tr>
                            ...
                            <td class="text-primary strong">{{ $transaction->invoice_no }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td>{{ trans('transaction.customer_name') }}</td>
                            <td>{{ $transaction->customer['name'] }}</td>
                        </tr>
                        <tr>
                            <td>{{ trans('transaction.customer_phone') }}</td>
                            <td>{{ $transaction->customer['phone'] }}</td>
                        </tr>
                        <tr>
                            <td>{{ trans('transaction.items_count') }}</td>
                            <td>{{ $transaction->items_count }}</td>
                        </tr>
                        <tr>
                            <td>{{ trans('transaction.total') }}</td>
                            <td class="text-right strong">{{ format_rp($transaction->total) }}</td>
                        </tr>
                        <tr>
                            <td>{{ trans('transaction.payment') }}</td>
                            <td class="text-right">{{ format_rp($transaction->payment) }}</td>
                        </tr>
                        <tr>
                            <td>{{ trans('transaction.exchange') }}</td>
                            <td class="text-right">{{ format_rp($transaction->getExchange()) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
